...
$(document).ready(function () {
    $('body').on('click', '.trumbowyg-modal-submit', function () {
        // $("body #trumbowyg-demo img").each(function(index, element) {
        //     $(element).wrap("<div class='redact-img'></div>");
        // });
        setTimeout(() => {
            $('#trumbowyg-demo img').each(function (index, item) {
                $(this).attr('image-num', index)
            })
        }, 10);
    })
    ...
    function resizeImg() {
        let num = null
        $('#trumbowyg-demo').on('click', 'img', function () {
            num = $(this).attr('image-num')
            $('.resize-img-modal').addClass('active')

        })
        $('.resize-img-modal').on('click', '.resize-submit', function (e) {
            e.preventDefault()
            e.stopPropagation()

            let width = $('.resize-form').find('input[name="width"]').val(),
                height = $('.resize-form').find('input[name="height"]').val()
            ...
            $('.resize-img-modal').removeClass('active')

        })
        $('.resize-img-modal').on('click', '.resize-cancel', function (e) {
            e.preventDefault()
            e.stopPropagation()
            $('.resize-img-modal').removeClass('active')
