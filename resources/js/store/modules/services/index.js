    ...
    actions: {
        async FETCH_STATISTICS({ state, commit }, { range }) {
            try {
                ...
                let { data } = await axios.get(
                    `/web-api/statistics?range=${_range}`
                );
                const additionalStatisticData = {
                    count: 0,
                    date: null
                };
                for (const key in data) {
                    if (data[key].length > 1) {
                        additionalStatisticData.date = data[key][1].date;
                    } else if (data[key].length === 1) {
                        data[key].push(additionalStatisticData);
                    }
                }
        ...
        async FETCH_INDIVIDUAL_STATISTICS({ state, commit }, { id, range }) {
            try {
                ...
                let { data } = await axios.get(
                    `/web-api/site/${_id}/statistics?range=${_range}`
                );
